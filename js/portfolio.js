...
//                'max-height': portfolio.height,
            ...
            $(this).find("img").first().attr('first', 'true').css({'margin-left': '20%'});
            $(this).find("img").last().attr('last', 'true').css({'margin-right': '20%'});
            // end
			
			// padd all images based on window width
			$(this).find("img").css('padding-left', ($(window).width()*.1));
            ...
            if (portfolio.lightbox) {
                ...
                $(gallery).find('img.active').animate({opacity: '1.3'});
